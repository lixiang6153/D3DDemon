...
const DWORD Vertex::FVF = D3DFVF_XYZ;   // flexible vertex format
...
bool CCubeShap::Init(HWND hWnd, int nWidth, int nHeight, bool bFullScreen /*= true*/)
{
	...
	if (NULL != pVertes)
	{
		pVertes[0] = Vertex(-1.0f, -1.0f, -1.0f);
		pVertes[1] = Vertex(-1.0f, 1.0f, -1.0f);
		pVertes[2] = Vertex(1.0f, 1.0f, -1.0f);
		pVertes[3] = Vertex(1.0f, -1.0f, -1.0f);
		pVertes[4] = Vertex(-1.0f, -1.0f, 1.0f);
		pVertes[5] = Vertex(-1.0f, 1.0f, 1.0f);
		pVertes[6] = Vertex(1.0f, 1.0f, 1.0f);
		pVertes[7] = Vertex(1.0f, -1.0f, 1.0f);
	...
	D3DXCreateTeapot(m_pDirect3DDevice, &m_pD3DMesh, 0);
...
void CCubeShap::Draw()
{
	...
	D3DXMATRIX stTransform;
	D3DXMatrixTranslation(&stTransform, 3.0f, 0.0f, 0.0f);
	D3DXMATRIX stWorld = rotationRet*stTransform;
	m_pDirect3DDevice->SetTransform(D3DTS_WORLD, &stWorld);
